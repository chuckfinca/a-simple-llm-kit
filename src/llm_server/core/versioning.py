...
async def get_versioning_info(
    ...
    versioning_info = {"request_id": str(uuid.uuid4()), "timestamp": get_utc_now()}
    ...
    if model_id:
        ...
        versioning_info["model"] = {"id": model_id, **model_info_dict}

    # Create structured program info
    if program_id:
        program_info = {"id": program_id, "version": program_version or "unknown"}

        if program_manager:
            program_metadata = program_manager.registry.get_program_metadata(
