            ...
            if message["type"] == "http.response.start":
                ...
                try:
                    ...
                    for field, error_message in self.required_fields.items():
                        ...
                        if (
                            field == "program_id"
                            and "program" in metadata
                            and "id" in metadata["program"]
                        ):
                            continue
                        elif (
                            field == "program_version"
                            and "program" in metadata
                            and "version" in metadata["program"]
                        ):
                            continue
                        elif (
                            field == "model_id"
                            and "model" in metadata
                            and "id" in metadata["model"]
                        ):
                            continue
                    ...
                    logging.warning(
                        f"Could not validate versioning info in response: {str(e)}"
                    )
