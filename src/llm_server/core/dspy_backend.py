            ...
            try:
                ...
                if self.program_manager and self.program_metadata:
                    (
                        ...
                        execution_info,
                        raw_completion_text,
                    ) = await self.program_manager.execute_program(
                    ...
                    result.raw_completion = raw_completion_text

                    metadata = getattr(result, "metadata", {})
                    metadata["execution_info"] = execution_info.model_dump()
                    metadata["program_metadata"] = ensure_program_metadata_object(
                ...
                logging.warning(
                    f"API call failed: {str(e)}, retrying in {delay}s...",
                    extra={"trace_id": trace_id},
                )
...
def create_dspy_backend(
    ...
    return DSPyModelBackendWithProcessor(
        model_manager, model_id, signature_class, output_processor, program_manager
    )
