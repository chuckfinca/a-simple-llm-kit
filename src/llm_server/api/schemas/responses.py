...
class StandardResponse(pydantic.BaseModel, Generic[T]):
    ...
    error: Optional[Union[str, dict[str, Any]]] = pydantic.Field(
        default=None, description="Error message or object if success is false"
    )
    timestamp: datetime = pydantic.Field(default_factory=get_utc_now)
    metadata: dict[str, Any] = pydantic.Field(
        default_factory=dict,
        description="Response metadata including program/model info",
    )
