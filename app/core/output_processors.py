...
class ContactExtractorProcessor:
    def _extract_json_from_text(self, text: str, field_name: str) -> Dict[str, Any]:
        pattern = re.compile(
            r"\[\[ ## " + re.escape(field_name) + r" ## \]\]\s*```json\s*([\s\S]*?)\s*```",
            re.DOTALL
        )
    ...
    def process(self, result: Any) -> ExtractContact:
        ...
        notes_match = re.search(r"\[\[ ## notes ## \]\]\s*```json\s*([\s\S]*?)\s*```", raw_output_text, re.DOTALL)
        final_notes = None
        if notes_match:
            notes_str = notes_match.group(1).strip()
            if notes_str.lower() != 'null':
                try: final_notes = json.loads(notes_str)
                except json.JSONDecodeError: final_notes = notes_str
        ...
        try:
            contact = ExtractContact(
                ...
                contact=ContactInformation(**contact_data) if contact_data else ContactInformation(),
